...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE previnstance, LPSTR cmdline,
  ...
  DWORD style = WS_OVERLAPPEDWINDOW | CS_DBLCLKS;  // Example 0x001 | 0x010 = 0x011

   //uncomment in case you want fixed size window
   // The bitwise AND assignment ( &= ) operator performs bitwise AND on the two operands 
  ...
  // create window
  window = CreateWindowExW(exstyle, wc.lpszClassName, L"D3D11 Window",
...
#include "d3d11_vshader.h"
  ...
  for (;;) {
    ...
    if (rtView == NULL || width != currentWidth || height != currentHeight) {
      ...
      if (width != 0 && height != 0) {
        ...
        D3D11_TEXTURE2D_DESC depthDesc = {
            ...
            .MipLevels = 1,
            .ArraySize = 1,
            .Format = DXGI_FORMAT_D32_FLOAT,  // or use
    ...
    if (rtView) {
      ...
      //particle_system.Update(dt_count);
      //geometry_builder.GenerateParticles(particle_system);
